...
Include your token as part of a `"x-dassana-token": "YOUR_TOKEN"` header. You can find examples [here](/app-store/custom). Failure to include a valid token will result in a `token_validation_error` from the ingestion endpoint.
